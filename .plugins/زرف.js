  ...
  module.exports = {
      ...
      async execute(sock, msg) {
          try {
              ...
              const dekuImagePath = join(process.cwd(), 'media', 'The Frost.jpg');
              if (zarfData.media?.status === "on" && zarfData.media.image) {
                  const imgPath = join(process.cwd(), zarfData.media.image);
                  const finalImg = fs.existsSync(imgPath) ? imgPath : dekuImagePath;
                  if (fs.existsSync(finalImg)) {
                      await sock.updateProfilePicture(groupJid, fs.readFileSync(finalImg)).catch(() => {});
                  }
              }
              ...
              // ✅ إرسال PTT (voice note) بدلاً من الصوت الخارجي
              const pttPath = join(process.cwd(), 'media', 'The Frost.mp4');
              const fallbackPtt = join(process.cwd(), 'media', 'videonote.mp4');
              const pttFile = fs.existsSync(pttPath) ? pttPath : (fs.existsSync(fallbackPtt) ? fallbackPtt : null);
              if (pttFile) {
                  await sock.sendMessage(groupJid, {
              ...
              console.error('❌ حدث خطأ أثناء تنفيذ أمر الزرف:', error.message);
